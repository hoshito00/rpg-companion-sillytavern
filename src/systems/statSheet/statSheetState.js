...
function _migrate() {
    ...
    // ── Session 14: gear + gearSlots ─────────────────────────────────────────
    // Full field backfill is handled lazily by ensureGearSlots() in gearTab.js.
    // Migration only needs to seed the arrays so STAT_SHEET_CHAT_FIELDS can save them.
    ...
    if (!ss.speedDice || typeof ss.speedDice !== 'object') {
        ...
        if (ss.speedDice.attrId   == null) { ss.speedDice.attrId   = '';    dirty = true; }
    }
...
function _defaultSpeedDice() {
    return { enabled: false, count: 1, sides: 6, modifier: 0, attrId: '' };
}
